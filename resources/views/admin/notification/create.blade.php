...
@section('content')
    ...
    <div class="content">
        <!-- BEGIN: Top Bar -->
        @include('admin.layouts.topbar', ['title' => 'Create Notifications'])
        <!-- END: Top Bar -->
        ...
        @if( !is_null( $errors->all() ) )
            ...
            @endforeach
        @endif
        <form method="POST" action="{{route('admin.notifications.store')}}" enctype="multipart/form-data" id="notificationForm">
            @csrf
            @method('POST')
                        ...
                        <div class="intro-y box p-5">
                            <div>
                                ...
                                <input id="crud-form-1" required name="title" type="text" class="form-control w-full"
                                       placeholder="Notification Title" value="{{ old('title') }}">
                            </div>
                            ...
                            <textarea name="body" id="descId" class="editor"
                                      placeholder="Description of the Notification">{{ old('body') }}</textarea>
                                </div>
                            </div>
