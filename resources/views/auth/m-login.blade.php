@extends('layouts.auth', ['type' => 'sign in with membership'])

@section('content')
    <form method="POST" action="{{ route('m-login') }}">
        @csrf
        ...
        <div class="intro-x mt-8">
            <input type="text" class="intro-x login__input form-control py-3 px-4 block" name="email" value="{{old('email')}}" placeholder="Membership Email">
            <input type="password" class="intro-x login__input form-control py-3 px-4 block mt-4" name="password" placeholder="Membership Password">
        </div>
        <div class="intro-x flex text-slate-600 dark:text-slate-500 text-xs sm:text-sm mt-4">
            ...
            <a href="{{route('login')}}">Login with PAN Event Details?</a>
        </div>
        <div class="intro-x flex text-slate-600 dark:text-slate-500 text-xs sm:text-sm mt-4">
