@extends('layouts.auth', ['type' => 'reset password'])
...
@section('content')
    <form method="POST" action="{{ route('password.store') }}">
        ...
        <div class="intro-x mt-8">
            ...
            <input type="text" class="intro-x login__input form-control py-3 px-4 block" name="email" value="{{ old('email', $request->email)  }}" placeholder="Email" readonly>
            <input type="password" class="intro-x login__input form-control py-3 px-4 block mt-4" name="password" placeholder="Password">
            <input type="password" class="intro-x login__input form-control py-3 px-4 block mt-4" name="password_confirmation" placeholder="Confirm Password">
        </div>
